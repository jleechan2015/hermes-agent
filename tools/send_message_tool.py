...
SEND_MESSAGE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "target": {
                "type": "string",
                "description": "Delivery target. Format: 'platform' (uses home channel), 'platform:#channel-name', 'platform:chat_id', or 'platform:chat_id:thread_id' for Telegram topics and Discord threads. Examples: 'telegram', 'telegram:-1001234567890:17585', 'discord:999888777:555444333', 'discord:#bot-home', 'slack:#engineering', 'signal:+155****4567', 'qqbot:channel_id'"
            },
            "message": {
                "type": "string",
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None):
    ...
    for chunk in chunks:
        if platform == Platform.DISCORD:
            ...
            result = await _send_qqbot(pconfig.extra, pconfig.token, chat_id, chunk)
        else:
...
async def _send_qqbot(extra, token, chat_id, message):
    ...
    and post a message. Works for guild channels and DMs without requiring
    ...
    appid = (extra or {}).get("appid") or os.getenv("QQBOT_APPID", "")
    secret = token or (extra or {}).get("secret") or os.getenv("QQBOT_SECRET", "") or os.getenv("QQBOT_TOKEN", "")
    if not appid or not secret:
        return _error("QQBot: QQBOT_APPID / QQBOT_SECRET not configured.")
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            if token_resp.status_code != 200:
                return _error(f"QQBot token request failed: {token_resp.status_code} {token_resp.text}")
            token_data = token_resp.json()
            access_token = token_data.get("access_token")
            if not access_token:
                return _error(f"QQBot: no access_token in response: {token_data}")
            ...
            # Default to channel message (guild channels / cron delivery)
            url = f"https://api.sgroup.qq.com/channels/{chat_id}/messages"
            payload = {"content": message[:2000], "msg_type": 0}
